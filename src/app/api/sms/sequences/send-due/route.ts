/**
 * Process due SMS sequence enrollments (Twilio).
 *
 * Auth: Authorization: Bearer <CRM_CRON_SECRET|SOCIAL_CRON_SECRET>
 *   or x-crm-cron-secret / x-social-cron-secret header.
 *
 * Degrades cleanly when Twilio secrets are missing — no fake "sent" rows.
...
export async function POST(req: NextRequest) {
    try {
            ...
            return NextResponse.json(
                {
                    ...
                    message:
                        "Due SMS enrollments stay queued. Set a cron secret and Twilio keys, or use Send next manually.",
            ...
            return NextResponse.json(
                {
                    ...
                    missingConfig: true,
                    message:
                        "Set TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_FROM_NUMBER. Due enrollments were not marked sent.",
                    processed: 0,
                },
                { status: 503 }
            );
        ...
        for (const enrollment of due || []) {
            ...
            const recipient = await resolveRecipientForSmsEnrollment(
                supabaseAdmin,
                enrollment
            );
            ...
            const result = await sendNextSmsSequenceStep(supabaseAdmin, {
        ...
        return NextResponse.json({
            ...
            meta: {
                ...
                message: "Processed due SMS sequence enrollments.",
            },
        });
    } catch (error: unknown) {
        console.error("sms-sequences send-due:", error);
        return NextResponse.json(
            {
                error:
